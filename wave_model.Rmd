...
wind.arrows <- clean_data %>% 
    filter(x %in% sort(unique(x))[c(TRUE, FALSE, FALSE, F, F, F, F, F, F, F, F)], 
           y %in% sort(unique(y))[c(TRUE, FALSE, FALSE, F, F, F, F, F, F, F, F)])

ggplot(data = clean_data, aes(frame = date)) +
  geom_raster(aes(y=y, x=x, 
                  fill = height), interpolate = T) +
  ...
  theme_classic() +
  geom_sf(data = islands, color = "black", fill = "grey60") +
